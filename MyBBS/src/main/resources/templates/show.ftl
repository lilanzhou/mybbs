            ...
            <div class="navbar-inner">
                <div class="container-fluid">
                    <a class="brand" href="http://www.papaok.org"><i
                            class="icon-home icon-white"></i> BBS论坛 <span class="sml_t">2.0</span></a>
...
<script type="text/javascript">
    ...
    function login() {
        ...
        var sun=$('#sun').is(':checked')==true?"on":"off";
        $.ajax({
            type: "post",
            url: "/user/login/" + username+ "/" + password+"/"+sun,
            dataType: "text",
            success: function (data) {
               // console.log("success")
                if(data.indexOf("0")!=-1){//找到了0
                   alert("登录失败，请检查你的密码或用户名是否正确")
                }else {

                    $.cookie("papaoku",$("#username").val(),{path:'/',expires:3600*24*7});
                    $.cookie("papaoku",$("#username").val(),{path:'/',expires:3600*24*7});
    ...
    function consumerMsg() {
        ...
        $.ajax({
            ...
            success: function (data) {
               // console.log(data)
                $(data).each(function (index) {//有消息就遍历提示
                    $.bootstrapGrowl("你的帖子"+data[index].title+"被回复了！", {
                        type: "danger",
                        width:'auto',
                        auto_dismiss:true  //自动关闭
    ...
    /*
        //id主贴
        //uid:浏览用户id
        //duid:发本帖的用户id，由于是从rshow来的，因此duid就是发主贴的用户id
